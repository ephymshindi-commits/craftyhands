...
export default function Navbar({ cartCount, onCartOpen, wishlistCount, dark, onToggleDark, logoImg }) {
    ...
    <>
      <nav className={`${styles.nav} ${scrolled ? styles.scrolled : ''}`}>
        ...
        <Link to="/" className={styles.logoWrap}>
          {logoImg
            ? <img src={logoImg} alt={config.brand.name} className={styles.logoImg}/>
            : null
          }
          {!logoImg && (
        ...
        <div className={styles.actions}>
          ...
          <button className={styles.iconBtn} onClick={onToggleDark} aria-label="Toggle dark mode">
            {dark ? (
              <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8">
      ...
      <div className={`${styles.mobileMenu} ${menuOpen ? styles.open : ''}`}>
        ...
        <div className={styles.mobileSocials}>
          <a href={`https://instagram.com/${config.contact.instagram}`} target="_blank" rel="noreferrer">Instagram</a>
          <a href={`https://wa.me/${config.contact.whatsapp}`} target="_blank" rel="noreferrer">WhatsApp</a>
        </div>
      </div>
    </>
  );
